...
class UIController:
    ...
    @staticmethod
    def upload_model(engine: FunctionGemmaEngine, repo_name: str) -> str:
        oauth_token = None  # 原代码中缺失的变量，临时置空
    ...
    # 在 UIController 类中新增静态方法
    @staticmethod
    def run_inference_demo(engine: FunctionGemmaEngine, prompt: str) -> str:
        """处理推理演示的逻辑"""
        if not engine:
            return "⚠️ 引擎未初始化，请先加载模型！"
        if not prompt.strip():
            return "⚠️ Prompt不能为空！"
        return engine.run_inference(prompt)

    @staticmethod
    def load_demo_prompts(engine: FunctionGemmaEngine) -> List[str]:
        return [
            "北京今天气温多少？",
            "125*8+99等于多少？",
            "查询公司内部文档",
    ...
    @staticmethod
    def select_demo_prompt(selected_prompt: str) -> str:
        """选择下拉列表中的Prompt并填充到输入框"""
...
def build_interface() -> gr.Blocks:
    with gr.Blocks(title="FunctionGemma Tuning Lab") as demo:
        ...
        # 新增推理按钮引用
        run_inference_btn = data_ui["run_inference_btn"]
        ...
        demo.load(lock_ui, outputs=action_buttons + [push_btn, zip_btn]).then(
            ...
            # 加载演示Prompt到下拉框
            fn=UIController.load_demo_prompts,
            inputs=[engine_state],
            outputs=[data_ui["demo_prompt_select"]]
        ...
        # ========== 新增推理演示面板事件绑定 ==========
        # 选择演示Prompt后填充到输入框
        data_ui["demo_prompt_select"].change(
            fn=UIController.select_demo_prompt,
            inputs=[data_ui["demo_prompt_select"]],
            outputs=[data_ui["custom_prompt"]]
        )

        # 执行推理
        run_inference_btn.click(
            fn=lambda: [gr.update(interactive=False) for _ in action_buttons + [push_btn, zip_btn, run_inference_btn]],
            outputs=action_buttons + [push_btn, zip_btn, run_inference_btn],
        ).then(
            fn=UIController.run_inference_demo,
            inputs=[engine_state, data_ui["custom_prompt"]],
            outputs=[data_ui["inference_output"]]
        ).then(
            fn=lambda: [gr.update(interactive=True) for _ in action_buttons + [push_btn, zip_btn, run_inference_btn]],
            outputs=action_buttons + [push_btn, zip_btn, run_inference_btn],
        )
